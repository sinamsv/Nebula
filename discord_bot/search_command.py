...
class SearchCommand(commands.Cog):
    def __init__(self, bot):
        ...
        self.auth = bot.auth
        self.coin_manager = bot.coin_manager
    ...
    async def search_command(self, ctx, *, query: str):
        ...
        if self.coin_manager:
            spend_result = self.coin_manager.check_and_spend(identity['nebula_user_id'], self.coin_manager.SEARCH_COST)
            if not spend_result['success']:
                await ctx.send(
                    self.coin_manager.insufficient_funds_message(ctx.author.display_name, spend_result['seconds_until_reset'])
                )
                return
