...
class AuthCommands(commands.Cog):
    ...
    @app_commands.command(name='signup', description='Create your Nebula account (shared across Discord, Telegram, etc).')
    ...
    async def signup_command(self, interaction: discord.Interaction, username: str, password: str, bootstrap_key: str = None):
        await interaction.response.defer(ephemeral=True)

        try:
            result = self.auth.signup(
                username=username, password=password, display_name=interaction.user.display_name,
                platform=DISCORD_PLATFORM, platform_user_id=str(interaction.user.id),
                platform_display_name=interaction.user.display_name, bootstrap_key=bootstrap_key,
            )
    ...
    @app_commands.command(name='login', description='Log in to your existing Nebula account from this platform.')
    @app_commands.describe(username='Your Nebula username', password='Your Nebula password')
    async def login_command(self, interaction: discord.Interaction, username: str, password: str):
        await interaction.response.defer(ephemeral=True)

        try:
            result = self.auth.login(
                username=username, password=password, platform=DISCORD_PLATFORM,
                platform_user_id=str(interaction.user.id), platform_display_name=interaction.user.display_name,
            )
        except AuthError as e:
            await interaction.followup.send(str(e), ephemeral=True)
            return
